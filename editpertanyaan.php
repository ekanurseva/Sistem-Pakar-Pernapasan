...
$data = query("SELECT * FROM pertanyaan WHERE idpertanyaan = $idpertanyaan")[0];

if (isset($_POST['submit_pertanyaan'])) {
  if (edit_pertanyaan($_POST) > 0) {
    // echo "
    //             <script>
  ...
  <div class="main-container d-flex">
    ...
    <div class="content" style="width:80%;">
      <div class="container" style="padding-left: 35px; padding-right: 20px;">
        <h1 style="text-align:center; margin-top: 30px; color: black; padding: 0px 35px">Edit Data Pertanyaan</h1>
        <form action="" method="post">
          <input type="hidden" name="idpertanyaan" value="<?= $data['idpertanyaan']; ?>">
          <input type="hidden" name="oldidgejala" value="<?= $data['idgejala']; ?>">
          <input type="hidden" name="oldkode_pertanyaan" value="<?= $data['kode_pertanyaan']; ?>">
          <input type="hidden" name="oldnama_pertanyaan" value="<?= $data['pertanyaan']; ?>">
          ...
          <div class="mb-3">
            ...
            <select class="form-control" name="gejala" require>
              ...
              while ($g = mysqli_fetch_array($gejala)) {
                ?>
                <option value="<?php echo $g['idgejala'] ?>" <?php echo ($g['idgejala'] == $data['idgejala']) ? 'selected' : ''; ?>><?php echo $g['kode_gejala']; ?>-<?php echo $g['nama_gejala'] ?></option>
              <?php } ?>
            </select>
          </div>
          <div class="mb-3">
            <label class="form-label">Pertanyaan</label>
            <input type="text" class="form-control" value="<?php echo $data['pertanyaan']; ?>" name="pertanyaan">
          </div>
          <div class="mb-3">
            <label class="form-label">Kode Pertanyaan</label>
            <input type="text" class="form-control" value="<?php echo $data['kode_pertanyaan']; ?>"
              name="kode_pertanyaan">
          </div>
          <button type="submit" name="submit_pertanyaan" class="btn btn-primary">Update</button>
        </form>
      </div>
    </div>
